...
public class MgrActivity extends BaseActivity {
	...
	private void initViews() {
		...
		networkChangeReceiver.addOnNetWorkChangeListener(new Function1<Boolean, Unit>() {
			@Override
			public Unit invoke(Boolean aBoolean) {
				if (aBoolean){
					Toast.makeText(MgrActivity.this,"网络已连接",Toast.LENGTH_SHORT).show();
				}else {
					Toast.makeText(MgrActivity.this,"网络不可用，请检查网络",Toast.LENGTH_SHORT).show();
				}
				return null;
			}
		});
	...
	private void enterMeetingActivity(int meetID, boolean createMeeting) {
		...
		MeetingActivity.Companion.setMMeetID(meetID);
		MeetingActivity.Companion.setMBCreateMeeting(createMeeting);
		startActivity(intent);
	}
