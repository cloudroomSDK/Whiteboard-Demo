    ...
    sdkLogin(cookie) {
      ...
      if (this.isInitSDK) {
        ...
        if (this.loginData.authType == '1') {
          const appID = this.loginData.appID == '默认appID' ? `demo@${window.__CRName}.com` : this.loginData.appID;
          const appSecret = this.loginData.appSecret == '默认appSecret' ? '123456' : this.loginData.appSecret;
          // SDK接口：登陆SDK
          CRVideo_Login(appID, md5(appSecret), this.loginData.nickname, this.loginData.userID, this.loginData.userAuthCode, cookie);
        } else {
          // SDK接口：登陆SDK（Token鉴权）
